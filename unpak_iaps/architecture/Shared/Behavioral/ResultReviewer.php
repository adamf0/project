    ...
    public function handle() {
        if($this->item->GetListSubstansi()->count()==0) $this->output .= null;
            ...
            if($this->render=="table"){
                $totalData  = $this->item->GetListSubstansi()->count();
                $table      = new Table();
                ...
                foreach($this->item->GetListSubstansi() as $substansi){
                    ...
                    if(
                        ...
                        $type = match($this->type){
                            "nasional"  => "NasionalReviewer",
                            "internal"  => "InternalReviewer",
                            "pkm"       => "PKMReviewer",
                            default     => "PKMNasionalReviewer",
                        };
                        $key = match($this->type){
                            "nasional"  => "idPDP",
                            "internal"  => "idPDP",
                        ...
                        $row->add(
                            new TableCell("<a href='".route("api.$type.removeAssign",["id"=>$substansi->GetId(), ($key)=>$this->item->GetId()])."' class='btn btn-sm btn-danger btn-delete-reviewer'><i class='bi bi-trash'></i></a>")
                        );
                    }
                    $table->add($row);
                }
